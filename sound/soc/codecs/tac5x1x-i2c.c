...
#define print_regs() \
	do { \
		...
		dev_dbg(tac5x1x->dev, "REGS: PWR_CFG=0x%x CH_EN=0x%x", val1, val2); \
	} while (0);
...
static void post_powerup_work(struct work_struct *work)
{
	...
	dev_dbg(tac5x1x->dev, "post_powerup: CH_EN 0x%02x 0x%02x",
		TAC5X1X_CH_EN,  tac5x1x->ch_enabled);
	snd_soc_component_write(component, TAC5X1X_CH_EN, tac5x1x->ch_enabled);

	regmap_read(tac5x1x->regmap, TAC5X1X_INTF4, &input_mode);

	/* check if it pdm mode */
	ch1_is_pdm = (input_mode & BIT(7)) ? true : false;
	ch2_is_pdm = (input_mode & BIT(6)) ? true : false;
	dev_dbg(tac5x1x->dev, "Input mode: 0x%02x, CH1_PDM=%d, CH2_PDM=%d\n",
	...
	if (ch1_is_pdm || ch2_is_pdm) {
		/* PDM mode - need ADC_PDZ, no MICBIAS */
		pwr_cfg &= ~TAC5X1X_PWR_CFG_MICBIAS;
		dev_dbg(tac5x1x->dev, "PDM: ADC_PDZ on, MICBIAS off, ch_en=0x%x",
			 tac5x1x->ch_enabled);
	} else if (IS_ADC_CH(tac5x1x->ch_enabled)) {
		/* Analog ADC mode - keep ADC_PDZ and MICBIAS */
		dev_dbg(tac5x1x->dev, "Analog: ADC_PDZ on, MICBIAS on");
	} else {
		/* No ADC/PDM - disable ADC power */
					 ...
					 bool is_adc, s32 right)
{
	...
	dev_dbg(tac5x1x->dev, "enable_ch: adc=%d r=%d mask=0x%x dev=0x%x en=0x%x", is_adc, right, mask, mask_dev, tac5x1x->ch_enabled);
					    ...
					    bool is_adc, s32 right)
{
	...
	tac5x1x->ch_enabled &= ~mask;
	dev_dbg(tac5x1x->dev, "disable_ch: adc=%d r=%d mask=0x%x en=0x%x", is_adc, right, mask, tac5x1x->ch_enabled);
			 ...
			 enum tac5x1x_type type)
{
	...
	case TAA5412:
		...
		dai->name = devm_kasprintf(dev, GFP_KERNEL, "taa5x1x-hifi-%02x-%02x",
