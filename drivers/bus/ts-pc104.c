...
struct tspc104_bus {
	...
	/*
	 * This the standard pinout, but supports a pinout that allows 16-bit
	 * cycles without the 40 pin header just using 64 pins.
	 */
	int use_ts_mode;
	void __iomem *reg;
	spinlock_t lock;
...
static int technologic_isa_probe(struct platform_device *pdev)
{
	...
	bus->reg = devm_ioremap(dev, mem->start, resource_size(mem));
	if (!bus->reg) {
